#!/usr/bin/env bash
log_success() {
  local flac="$1" wv="$2" md5="$3" sha="$4" notes="${5:-}"
  ...
  case "${SUCCESS_LOG}" in
    *.jsonl)
      append_locked "${SUCCESS_LOG}" \
        '{"ts":"%s","flac":%s,"wv":%s,"audio_md5":"%s","wv_sha256":"%s","codec":%s,"bytes":%s,"samples":%s,"notes":%s}\n' \
        "$ts" "$(json_str "$flac")" "$(json_str "$wv")" "$md5" "$sha" \
        "$(json_str "${codec:-}")" "${bytes:-0}" "$(json_str "${samples:-}")" "$(json_str "$notes")"
      ;;
    *)
      append_locked "${SUCCESS_LOG}" '%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
        "$ts" "$(csv_escape "$flac")" "$(csv_escape "$wv")" "$md5" "$sha" \
...
init_success_log() {
  ...
  case "${SUCCESS_LOG}" in
    *.jsonl) ;;
    *) printf '%s\n' 'timestamp,flac,wv,audio_md5,wv_sha256,codec,bytes,samples,notes' >"${SUCCESS_LOG}"
