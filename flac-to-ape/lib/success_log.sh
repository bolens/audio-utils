  ...
  case "${SUCCESS_LOG}" in
    *.jsonl)
      append_locked "${SUCCESS_LOG}" \
        '{"ts":"%s","flac":%s,"ape":%s,"audio_md5":"%s","ape_sha256":"%s","codec":%s,"bytes":%s,"samples":%s,"notes":%s}\n' \
...
init_success_log() {
  ...
  case "${SUCCESS_LOG}" in
    *.jsonl) ;;
    *) printf '%s\n' 'timestamp,flac,ape,audio_md5,ape_sha256,codec,bytes,samples,notes' >"${SUCCESS_LOG}"
