  ...
  case "${SUCCESS_LOG}" in
    *.jsonl)
      append_locked "${SUCCESS_LOG}" \
        '{"ts":"%s","aiff":%s,"wav":%s,"audio_md5":"%s","wav_sha256":"%s","codec":%s,"bytes":%s,"samples":%s,"notes":%s}\n' \
...
init_success_log() {
  ...
  case "${SUCCESS_LOG}" in
    *.jsonl) ;;
    *) printf '%s\n' 'timestamp,aiff,wav,audio_md5,wav_sha256,codec,bytes,samples,notes' >"${SUCCESS_LOG}"
