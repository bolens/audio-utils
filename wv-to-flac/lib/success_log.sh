#!/usr/bin/env bash
log_success() {
  local wv="$1" flac="$2" md5="$3" sha="$4" notes="${5:-}"
  local codec bytes samples ts
  [[ "${DRY_RUN:-0}" -eq 1 || -z "${SUCCESS_LOG:-}" ]] && return 0
  IFS=$'\t' read -r codec bytes samples < <(probe_debug_fields "$wv")
  ts=$(date -Iseconds)
  case "${SUCCESS_LOG}" in
    *.jsonl)
      append_locked "${SUCCESS_LOG}" \
        '{"ts":"%s","wv":%s,"flac":%s,"audio_md5":"%s","flac_sha256":"%s","codec":%s,"bytes":%s,"samples":%s,"notes":%s}\n' \
        "$ts" "$(json_str "$wv")" "$(json_str "$flac")" "$md5" "$sha" \
        "$(json_str "${codec:-}")" "${bytes:-0}" "$(json_str "${samples:-}")" "$(json_str "$notes")"
      ;;
    *)
      append_locked "${SUCCESS_LOG}" '%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
        "$ts" "$(csv_escape "$wv")" "$(csv_escape "$flac")" "$md5" "$sha" \
...
init_success_log() {
  ...
  case "${SUCCESS_LOG}" in
    *.jsonl) ;;
    *) printf '%s\n' 'timestamp,wv,flac,audio_md5,flac_sha256,codec,bytes,samples,notes' >"${SUCCESS_LOG}"
