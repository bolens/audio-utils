#!/usr/bin/env bash
# audio-replaygain — ReplayGain for FLAC + common lossy formats.

AU_TOOL_NAME="${AU_TOOL_NAME:-audio-replaygain}"
AU_SOURCE_EXT=flac
AU_SOURCE_EXTS="flac mp3 opus m4a ogg oga wma mpc spx aac"
AU_DEST_EXT=flac
AU_DISK_FACTOR=0
AU_WORKDIR_PREFIX=audiorg
...
plugin_consume_arg() {
  case "${1:-}" in
    --track)
      RG_TRACK_ONLY=1; AU_CONSUMED=1
      export AU_CONSUMED RG_TRACK_ONLY; return 0 ;;
  esac
  return 1
}
...
plugin_require_deps() {
  require_cmds flock ffmpeg ffprobe
...
plugin_export_env() {
  ...
  export AU_RG_STATE_DIR RG_TRACK_ONLY RG_BACKEND AU_CLEANUP_SKIP AU_SOURCE_EXTS
}
