  ...
  case "${SUCCESS_LOG}" in
    *.jsonl)
      append_locked "${SUCCESS_LOG}" \
        '{"ts":"%s","flac":%s,"m4a":%s,"src_audio_md5":"%s","m4a_sha256":"%s","codec":%s,"bytes":%s,"samples":%s,"quality":%s,"notes":%s}\n' \
...
init_success_log() {
  ...
  case "${SUCCESS_LOG}" in
    *.jsonl) ;;
    *)
      printf '%s\n' 'timestamp,flac,m4a,src_audio_md5,m4a_sha256,codec,bytes,samples,quality,notes' >"${SUCCESS_LOG}"
