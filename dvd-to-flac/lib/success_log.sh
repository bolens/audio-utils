  ...
  case "${SUCCESS_LOG}" in
    *.jsonl)
      append_locked "${SUCCESS_LOG}" \
        '{"ts":"%s","vob":%s,"flac":%s,"audio_md5":"%s","flac_sha256":"%s","codec":%s,"bytes":%s,"samples":%s,"notes":%s}\n' \
...
init_success_log() {
  ...
  case "${SUCCESS_LOG}" in
    *.jsonl) ;;
    *) printf '%s\n' 'timestamp,vob,flac,audio_md5,flac_sha256,codec,bytes,samples,notes' >"${SUCCESS_LOG}"
