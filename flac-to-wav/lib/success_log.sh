#!/usr/bin/env bash
# flac-to-wav success log (CSV / JSONL).

log_success() {
  local flac="$1" wav="$2" md5="$3" sha="$4" notes="${5:-}"
  ...
  case "${SUCCESS_LOG}" in
    *.jsonl)
      append_locked "${SUCCESS_LOG}" \
        '{"ts":"%s","flac":%s,"wav":%s,"audio_md5":"%s","wav_sha256":"%s","codec":%s,"bytes":%s,"samples":%s,"notes":%s}\n' \
        "$ts" \
        "$(json_str "$flac")" \
        "$(json_str "$wav")" \
        "$md5" \
...
init_success_log() {
  ...
  case "${SUCCESS_LOG}" in
    ...
    *)
      printf '%s\n' \
        'timestamp,flac,wav,audio_md5,wav_sha256,codec,bytes,samples,notes' \
        >"${SUCCESS_LOG}"
