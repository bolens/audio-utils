#!/usr/bin/env bash
# aiff-to-flac success log

log_success() {
  local aiff="$1" flac="$2" md5="$3" sha="$4" notes="${5:-}"
  local codec bytes samples ts
  [[ "${DRY_RUN:-0}" -eq 1 || -z "${SUCCESS_LOG:-}" ]] && return 0

  IFS=$'\t' read -r codec bytes samples < <(probe_debug_fields "$aiff")
  ts=$(date -Iseconds)

  case "${SUCCESS_LOG}" in
    *.jsonl)
      append_locked "${SUCCESS_LOG}" \
        '{"ts":"%s","aiff":%s,"flac":%s,"audio_md5":"%s","flac_sha256":"%s","codec":%s,"bytes":%s,"samples":%s,"notes":%s}\n' \
        "$ts" \
        "$(json_str "$aiff")" \
        "$(json_str "$flac")" \
        "$md5" \
...
init_success_log() {
  ...
  case "${SUCCESS_LOG}" in
    ...
    *)
      printf '%s\n' \
        'timestamp,aiff,flac,audio_md5,flac_sha256,codec,bytes,samples,notes' \
        >"${SUCCESS_LOG}"
