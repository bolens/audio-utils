...
convert_one() {
  ...
  local quality="${LOSSY_QUALITY_NAME:-q6}"

  if [[ -f "$out" && "${OVERWRITE:-0}" -eq 0 ]]; then
    if ogg_ok "$out"; then
      log_progress "skip (ogg ok): $out"
  ...
  if [[ "${DRY_RUN:-0}" -eq 1 ]]; then
    ...
    log_info "would encode:         libvorbis quality=$quality (${LOSSY_FF_ARGS[*]})"
  ...
  if ! lossy_prepare_source "$flac" "$tmpdir" "vorbis" >"${tmpdir}/prep.path"; then
    log_fail "$flac" "lossy prepare failed" "family=vorbis"
    cleanup
    return 1
  fi
  ...
  if ! encode_vorbis "$prep" "$enc_out"; then
    log_fail "$flac" "encode vorbis failed" "quality=$quality"
    cleanup
    return 1
  fi

  if ! ogg_ok "$enc_out"; then
    log_fail "$flac" "ogg probe failed after encode"
    cleanup
    return 1
  fi
