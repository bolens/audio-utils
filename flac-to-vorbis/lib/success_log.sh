  ...
  case "${SUCCESS_LOG}" in
    *.jsonl)
      append_locked "${SUCCESS_LOG}" \
        '{"ts":"%s","flac":%s,"ogg":%s,"src_audio_md5":"%s","ogg_sha256":"%s","codec":%s,"bytes":%s,"samples":%s,"quality":%s,"notes":%s}\n' \
...
init_success_log() {
  ...
  case "${SUCCESS_LOG}" in
    *.jsonl) ;;
    *)
      printf '%s\n' 'timestamp,flac,ogg,src_audio_md5,ogg_sha256,codec,bytes,samples,quality,notes' >"${SUCCESS_LOG}"
