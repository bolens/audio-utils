  ...
  case "${SUCCESS_LOG}" in
    *.jsonl)
      append_locked "${SUCCESS_LOG}" \
        '{"ts":"%s","wav":%s,"aiff":%s,"audio_md5":"%s","aiff_sha256":"%s","codec":%s,"bytes":%s,"samples":%s,"notes":%s}\n' \
...
init_success_log() {
  ...
  case "${SUCCESS_LOG}" in
    *.jsonl) ;;
    *) printf '%s\n' 'timestamp,wav,aiff,audio_md5,aiff_sha256,codec,bytes,samples,notes' >"${SUCCESS_LOG}"
